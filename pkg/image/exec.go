...
type CmdRunner interface {
	...
	RunWithStream(ctx context.Context, callback func(string), name string, args ...string) error
}
...
func (r *defaultCmdRunner) Run(ctx context.Context, name string, args ...string) ([]byte, error) {
...
func (r *defaultCmdRunner) RunWithStream(ctx context.Context, callback func(string), name string, args ...string) error {
	c := exec.CommandContext(ctx, name, args...)
...
// qemuCmd provides a domain-specific API on top of CmdRunner
type qemuCmd struct {
	runner CmdRunner
}

func newQemuCmd(runner CmdRunner) *qemuCmd {
	return &qemuCmd{runner: runner}
}

// Exec runs qemu-img with the given args, discarding stdout
func (q *qemuCmd) Exec(args ...string) error {
	_, err := q.runner.Run(context.Background(), qemuImg, args...)
	return err
}
